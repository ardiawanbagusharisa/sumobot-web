...
export async function ensureMatchSchema() {
  ...
  const columns = await d1.prepare("PRAGMA table_info(prototype_match_records)").all<{ name: string }>();
  if (!columns.results.some((column) => column.name === "campaign")) {
    try {
      await d1.prepare("ALTER TABLE prototype_match_records ADD COLUMN campaign INTEGER").run();
    } catch (error) {
  ...
  if (featuredPool.results.length < 3) {
    ...
    await d1.batch([
      ...
      d1.prepare(`INSERT OR IGNORE INTO prototype_match_records
        ...
        VALUES ('match-featured-demo', 'system-featured', 'sumobot', 'vector-demo', 'Vector', 'buttons', 'pvai', 'win', 1, '{}', ?, 0, ?)`)
        .bind(replayJson, HOME_DEMO_META.playedAt),
    ]);
    const recent = await d1.prepare(`SELECT id, replay, result, played_at AS playedAt
      FROM prototype_match_records WHERE player_id != 'system-featured' AND campaign = 0 AND battle_mode IN ('pvai', 'pvp')
...
export async function recordMatch(user: AuthUser, submission: MatchSubmission) {
  ...
  const historyEntry = {
    ...
    campaign: Boolean(submission.campaign),
  };
  ...
  await d1.batch([
    ...
    d1.prepare(`INSERT INTO prototype_match_records
      (id, player_id, player_handle, bot_id, bot_name, control_mode, battle_mode, result, rank_points, telemetry, replay, campaign, played_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(submission.id, user.id, user.handle, submission.botId, submission.botName, submission.controlMode, submission.battleMode, submission.result, points, telemetryJson, replayJson, submission.campaign ? 1 : 0, submission.playedAt),
  ]);
  return { id: submission.id, points, rewards, profile: nextProfile, revision: current.revision + 1 };
}

function parseFeaturedReplay(row: { replay: string; result: MatchResult; playedAt: string }) {
  try {
    const replay = JSON.parse(row.replay) as Record<string, unknown>;
    const arena = replay.arena as Record<string, unknown> | undefined;
    const frames = replay.frames;
    const validVersion = replay.version === 1 || replay.version === 2 || replay.version === 3;
    const validArena = arena && [arena.width, arena.height, arena.x, arena.y, arena.radius].every((value) => typeof value === "number" && Number.isFinite(value));
    const validFrames = Array.isArray(frames) && frames.length >= 2 && frames.every((frame) => Array.isArray(frame) && frame.length >= 15 && frame.slice(0, 15).every((value) => typeof value === "number" && Number.isFinite(value)));
    if (!validVersion || !validArena || !replay.player || !replay.enemy || !validFrames) return null;
...
export async function getFeaturedReplays() {
  ...
  const rows = await d1.prepare(`SELECT replay, result, played_at AS playedAt
    ...
    WHERE campaign = 0 AND battle_mode IN ('pvai', 'pvp') AND player_id != 'system-featured'
    ORDER BY RANDOM() LIMIT 24`)
    .all<{ replay: string; result: MatchResult; playedAt: string }>();
  const valid = rows.results.flatMap((row) => {
    const replay = parseFeaturedReplay(row);
    return replay ? [replay] : [];
  });
  return valid.length ? valid : [{ replay: HOME_DEMO_REPLAY as unknown as Record<string, unknown>, result: HOME_DEMO_META.result, playedAt: HOME_DEMO_META.playedAt }];
}
...
export function validateMatchSubmission(value: unknown): MatchSubmission | null {
  ...
  if (!item.battleMode || !["pvai", "pvp"].includes(item.battleMode) || !item.result || !["win", "draw", "loss"].includes(item.result)) return null;
