...
export function SumobotApp() {
    ...
    const [competitionBattle,setCompetitionBattle]=useState(false);
    ...
    const battleOrigin = (entry: BattleLog) => entry.competitionTitle ? `Competition · ${entry.competitionTitle}` : entry.campaign ? "Campaign mission" : entry.battleType === "pvai" ? "Regular battle · PvAI" : "Regular battle · PvP";
      ...
      <div className="battle-log-table">
        ...
        {selectedBotHistory.length === 0 ? <div className="battle-log-empty"><strong>No claimed battles for {hangarBot.name} yet.</strong><span>Select this bot before starting a battle to create its first log.</span></div> : [...selectedBotHistory].reverse().map((entry) => {
          ...
          return <div className="battle-log-row" key={entry.id}>
            ...
            <span><strong>{entry.replay?.player.name ?? loggedBot?.name ?? "Archived bot"}</strong><small>vs. {entry.battleType === "pvai" ? "Pebble" : "Rival"}</small><small className="battle-origin">{battleOrigin(entry)}</small></span>
...
{visibleNavItems.map((item) => <button key={item.id} className={view === item.id ? "active" : ""} type="button" onClick={() => navigate(item.id)}><span>{item.mark}</span>{item.label}</button>)}
    ...
    {view === "play" &&
<section className="content-page page-width play-page">{battleActive ? (battleType === "pvp" && onlineRoomId ? <OnlineBattleRoom roomId={onlineRoomId} bots={onlineBots} selectedBotId={battleBot.id} autoReturnSeconds={competitionBattle?30:undefined} returnLabel={competitionBattle?"Return to Seasons now":"Return to rooms"} onExit={() => { const returnToCompetition=competitionBattle;setOnlineRoomId(null);setBattleActive(false);setCompetitionBattle(false);if(returnToCompetition)setView("competitions");void refreshOnlineData(); }} onProfileChanged={() => void refreshOnlineData()} /> : <><div className="battle-page-heading"><button type="button" onClick={() => { setBattleActive(false); if (practiceBattle)
    ...
    {view === "competitions" && <CompetitionCenter bot={onlineBots.find((bot) => bot.id === battleBot.id) ?? onlineBots[0]} onNotice={showToast} onMatched={(roomId, roomMode) => { setMode(roomMode); setOnlineRoomId(roomId); setCompetitionBattle(true); setBattleType("pvp"); setBattleActive(true); setView("play"); }} />}
